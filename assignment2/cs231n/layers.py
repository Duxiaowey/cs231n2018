...
def affine_forward(x, w, b):
    ...
    num_input = x.shape[0]
    # dim = w.shape[0]
    # num_output = b.shape[0]
    x_compute = x.reshape(num_input, -1)
    out = x_compute.dot(w) + b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def batchnorm_backward(dout, cache):
    ...
    dsample_var = np.sum(-0.5 * dx_normalized * x_mu, axis=0, keepdims=True) * sample_std_inv ** 3
    dsample_mean = np.sum(- dx_normalized * sample_std_inv, axis=0, keepdims=True) + np.sum(
        - dsample_var * 2 * np.mean(x_mu, axis=0, keepdims=True))

    dx1 = dx_normalized * sample_std_inv
    dx2 = dsample_var * 2 * x_mu / N
    dx = dx1 + dx2 + dsample_mean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1 / N) * (1 / std_var) * (N * dxhat - np.sum(dxhat, axis=0) - x_normalized * np.sum(dxhat * dxhat, axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    # still apply!                                                            #
    ###########################################################################
    x_normalized, gamma, xmu, std_var, xvar, eps = cache
    dxhat = dout * gamma
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):  # nth image
        for k in range(F):  # kth filter
            for i in range(Hout):
                for j in range(Wout):
                    out[n, k, i, j] = np.sum(
                        xpad[n, :, i * stride: i * stride + HH, j * stride: j * stride + WW] * w[k]) + b[k]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
        ...
        for f in range(F):
            for hh in range(H_out):
                for ww in range(W_out):
                    dw[f] += dout[n, f, hh, ww] * x_padded[n, :, hh*stride:hh*stride+HH, ww*stride:ww*stride+WW]
                    dx_padded[n, :, hh*stride:hh*stride+HH, ww*stride:ww*stride+WW] += w[f] * dout[n, f, hh, ww]

    #transform from shape(x_padded) to shape(x)
        ...
        for c in range(C):
            for h in range(H_out):
                for w in range(W_out):
                    window = x[n, c, h*stride:h*stride+pool_height, w*stride:w*stride+pool_width]
                    m = np.max(window)
                    dx[n, c, h*stride:h*stride+pool_height, w*stride:w*stride+pool_width] = dout[n, c, h, w] * (window == m)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
